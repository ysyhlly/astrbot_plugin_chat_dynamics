...
- 决策快照增加独立结构化环境特征 E：@ 与回复机器人、机器人最近发言间隔及次数、近五分钟群活跃度和冷却剩余时间；缺失或不完整的信号记为 `null`。匿名化样本保留 E，离线 case 单独导出供后续融合训练。当前 AgentJev 服务尚未消费 E。
- 修复 AgentJev 训练与在线输入的候选身份、正文和路由证据一致性；补强群消息交付、成员停止、配置重载与运行时持久化保护，并改进学习页面的错误反馈。

## v1.12.1 · 修复决策训练快照完整性（2026-09-23）

- 修复 token 预算压缩时直接截取 JSON 尾部，导致训练输入残缺的问题；现在只移除完整可选历史条目，关键证据超预算明确拒绝。
- 学习快照保留逐条消息正文、时间、@ 和回复关系，增加目标与收件人的显式候选映射，并过滤晚于当前触发消息的历史。
- 另存完整匿名化源输入；无法适配小模型预算的样本保留完整教师输入，并标记尚未完成 tokenizer 准备。
- 新增快照审计与隔离命令。后台标注、导出、切分和训练入口拒绝残缺输入，不猜补旧记录中丢失的正文。
- 保持普通回复提示词的既有展示预算，以及 v1.12.0 的并行补标与领取锁行为。
...
- 采纳时优先使用内存里的消息；消息不在了就从快照重建同一条记录，写入的 `predicted_topic`、`routing` 与 `decision_trace` 与重启后重建的标注一致，不新增标注字段。
- 快照有上限：每条约 32 KB、每会话最多 512 条且合计 2 MB；装不下的草稿退回旧行为（仍列出，只能忽略），快照只补不改，后台再生成一轮不会让正在审批的页面失效。
- 审批页区分「已失效（只能忽略）」与「消息已不在内存、按快照采纳」，采纳结果回报其中多少条来自快照；快照不出现在任何页面响应里。
...
- 兼容 AstrBot 不允许同源访问的插件沙箱：会话存储不可用时在当前页面保留采纳请求 ID，继续支持请求核对；仅在仍有待确认结果时提示先核对再离开页面。
- 修复局域网 HTTP 下待审草稿采纳因 `crypto.randomUUID()` 不可用而未发送请求；会话存储失败明确提示并保持可重试，避免误报草稿失效。
- 预标注采用完整 JSON 分批和窗口身份映射，准确区分 @Bot 与 @他人；分别报告漏答、重复、窗外 ID 和非法置信度。
- 草稿生成校验代际、最新人工标签与忽略记录；清空后旧结果不能复活，显式重生成不覆盖人工标注。
- 审核写入支持持久化请求 ID、逐项重放及中断对账，前端保存待确认请求并支持刷新后核对；分片索引不再截断保留会话。
- 确定性回归与真实效果验收分离，合成有标签边界集纳入 CI；暂无独立真实人工标签时返回 `not_ready`。
- 实际参与策略消费不可变回合证据，历史 Trace 一次定稿；迟到推断只更新当前解释，发送结果独立补充。补齐 Persona 等待后停止/reset 校验。
...
- 配置只提交实际修改字段，并校验字段加载基线，防止旧页面覆盖其他页面的新设置；保护保存过程中的编辑及强制应用操作，严格校验数值并展开、定位错误字段。
- 回放标注绑定完整会话与消息身份，取消刷新不会替换编辑对象；慢保存后保留新输入的未保存状态。
- 草稿单条与批量采纳统一审核，校验人工标注版本并保留无关字段；批量按上限分块，保留未完成选择，勾选保持键盘焦点。
- 草稿采纳默认不审核话题；未审核话题与正确话题分别记录，话题评估排除未审核标签。
- 明确显示历史会话目标、配置学习策略来源；修复控制台档位即时刷新、导航取消后按钮状态及分寸台互斥操作反馈。
