...
`topic_reranker_enabled` 默认开启，对模糊候选及拟新建话题调用 LLM 聚合辅助，减少同一讨论被拆成多个话题。已显式设为 false 的配置仍保持关闭。`topic_reranker_provider` 留空使用当前会话模型，`topic_reranker_timeout` 默认 3 秒；失败或无法确认时保留本地判断；模型不会仅因相同参与者或泛技术类别而合并话题。
