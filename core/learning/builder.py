"""Turn an existing human annotation into labellable samples.

The replay annotations already store the decision trace, including the ledger,
so no new runtime collection path is needed and no message text is copied. Two
labelled tasks come out of one record: topic assignment, and whether the bot
should have been addressed. A third appears when the annotator recorded whether
a reply was wanted **and** the trace recorded which way the bot actually went.

Every row that is withheld for want of a fact is counted, and the reason is
reported rather than swallowed. A sample built on a missing fact would read as
supervision while being a guess.
"""
from __future__ import annotations

import hashlib
from dataclasses import dataclass, field
from collections.abc import Mapping
from math import isfinite

from ..message_features import analyze_text
from .sample import FEATURE_CODES, FEATURE_SCHEMA_VERSION, LearningSample

NONE = "none"
BOT = "bot"
OTHER = "other"
REPLY = "reply"
SILENT = "silent"

# Why a sample the annotation could have produced was not produced.
SKIP_NO_BOT_ID = "no_bot_id"
SKIP_NO_RECIPIENT_LABEL = "no_recipient_label"
SKIP_NO_REPLY_LABEL = "no_reply_label"
SKIP_UNDECIDED_REPLY = "undecided_reply"
SKIP_REASONS = (SKIP_NO_BOT_ID, SKIP_NO_RECIPIENT_LABEL, SKIP_NO_REPLY_LABEL,
                SKIP_UNDECIDED_REPLY)


def facts_from_text(text: str) -> dict:
    """Shape facts a labelled message carries, without keeping the text itself."""
    features = analyze_text(str(text or ""))
    return {
        "fact.is_short": float(features.is_short),
        "fact.is_elliptical": float(features.is_elliptical),
        "fact.is_ack": float(features.is_ack),
        "fact.is_question": float(features.is_question),
        "fact.is_answer_like": float(features.is_answer_like),
        "fact.is_topic_boundary": float(features.is_topic_boundary),
        "fact.is_filler": float(features.is_filler),
        "fact.can_start_topic": float(features.can_start_topic),
        "fact.question_ending": float(features.question_ending),
        "fact.answer_boundary": float(features.answer_boundary),
        "fact.reaction_like": float(features.is_filler or features.is_ack),
        "fact.information_density": float(features.information_density),
        "fact.lexical_terms": float(len(features.lexical_keywords)),
    }


def facts_from_trace(trace: Mapping | None, routing: Mapping | None = None) -> dict:
    """Identity facts the trace already recorded; none of them need the text."""
    identity = (trace or {}).get("identity")
    identity = identity if isinstance(identity, Mapping) else {}
    codes = (routing or {}).get("evidence", ())
    codes = codes if isinstance(codes, (list, tuple)) else ()
    return {
        "fact.bot_mentioned": float(bool(identity.get("mention"))),
        "fact.bot_vocative": float(bool(identity.get("vocative"))),
        "fact.bot_subject": float(bool(identity.get("subject"))),
        "fact.has_reply": float("explicit_reply" in codes),
    }


def features_from_trace(trace: Mapping | None, routing: Mapping | None = None, *,
                        text: str = "") -> tuple:
    """Raw factor values from the ledger; applied codes fall back to 1.0.

    The ledger stores "raw_value" (the measurement) separately from
    "contribution" (what the current policy did with it). Only the raw value is a
    feature: feeding the contribution back in would let a learner recover
    nothing but the weights that produced it.

    Message facts join only when the annotation kept the text, which depends on
    the console content switch. A fact that was never observed is **absent**
    rather than zero, so a factor table can always say how many rows carried it.
    """
    collected: dict = {}
    entries = (trace or {}).get("ledger", {})
    entries = entries.get("entries", ()) if isinstance(entries, Mapping) else ()
    for entry in entries if isinstance(entries, (list, tuple)) else ():
        if not isinstance(entry, Mapping):
            continue
        code, raw = entry.get("code"), entry.get("raw_value")
        if (code in FEATURE_CODES and isinstance(raw, (int, float))
                and not isinstance(raw, bool) and isfinite(raw)):
            collected[code] = float(raw)
    codes = (routing or {}).get("evidence", ())
    for code in codes if isinstance(codes, (list, tuple)) else ():
        if code in FEATURE_CODES:
            collected.setdefault(code, 1.0)
    collected.update(facts_from_trace(trace, routing))
    if isinstance(text, str) and text.strip():
        collected.update(facts_from_text(text))
    return tuple(sorted(collected.items()))


def session_hash(session_key: str) -> str:
    """Stable per-session grouping key that does not carry the group id.

    This is the same derivation the annotation store already uses for its
    per-session key, so a record and its stored annotations agree. A keyed HMAC
    would resist guessing better, but it needs a secret that has to be persisted
    and would stop the two from matching, so the existing convention is kept.
    """
    if not session_key:
        return ""
    return hashlib.sha256(str(session_key).encode("utf-8")).hexdigest()


def _confidence(trace: Mapping, domain: str) -> float:
    value = (trace.get(domain) or {}).get("confidence")
    return float(value) if isinstance(value, (int, float)) and not isinstance(value, bool) else 0.0


def _recipient_label(record: Mapping) -> str | None:
    value = record.get("bot_targeted")
    if value is True:
        return BOT
    if value is False:
        return OTHER
    return None


def _predicted_recipient(trace: Mapping, bot_id: str) -> str | None:
    """Unknown without the bot id: guessing would fabricate a perfect label."""
    if not bot_id:
        return None
    ids = (trace.get("recipient") or {}).get("ids") or []
    return BOT if any(str(item) == bot_id for item in ids) else OTHER


@dataclass
class SampleBuildReport:
    """Samples plus an account of everything that was withheld and why."""

    samples: list = field(default_factory=list)
    records: int = 0
    skipped: dict = field(default_factory=dict)
    ungroupable: int = 0

    def note(self, reason: str) -> None:
        self.skipped[reason] = self.skipped.get(reason, 0) + 1

    @property
    def skipped_total(self) -> int:
        return sum(self.skipped.values())

    def warnings(self) -> list:
        """Human-readable reasons, most useful first."""
        lines: list = []
        if self.skipped.get(SKIP_NO_BOT_ID):
            lines.append(
                f"{self.skipped[SKIP_NO_BOT_ID]} 条标注有收件人标签但缺少 bot id，"
                "收件人样本已跳过：没有 bot id 时路由器的选择读不出来，"
                "把它记成「不是对 Bot 说话」会得到一份看起来完美的语料")
        if self.skipped.get(SKIP_UNDECIDED_REPLY):
            lines.append(
                f"{self.skipped[SKIP_UNDECIDED_REPLY]} 条标注的 decision trace 没有记录"
                "明确的是否回复决策（should_reply 为 null），参与样本已跳过："
                "「未决」不是「应该沉默」")
        if self.skipped.get(SKIP_NO_REPLY_LABEL):
            lines.append(
                f"{self.skipped[SKIP_NO_REPLY_LABEL]} 条标注没有 expected_reply，参与样本已跳过")
        if self.skipped.get(SKIP_NO_RECIPIENT_LABEL):
            lines.append(
                f"{self.skipped[SKIP_NO_RECIPIENT_LABEL]} 条标注没有 bot_targeted，收件人样本已跳过")
        if self.ungroupable:
            lines.append(
                f"{self.ungroupable} 条样本没有 session 标识，无法做按会话分组验证；"
                "请从写入了 session_hash 的插件版本重新导出标注")
        return lines


def build_samples(records, *, session_id: str = "", bot_id: str = "",
                  source: str = "manual_replay",
                  plugin_version: str = "") -> SampleBuildReport:
    """Build samples from annotation records, counting every withheld row."""
    report = SampleBuildReport()
    for record in records if isinstance(records, (list, tuple)) else ():
        report.records += 1
        for sample in samples_from_annotation(record, session_id=session_id, bot_id=bot_id,
                                              source=source, plugin_version=plugin_version,
                                              report=report):
            report.samples.append(sample)
            if not sample.group_key:
                report.ungroupable += 1
    return report


def samples_from_annotation(record: Mapping, *, session_id: str = "", bot_id: str = "",
                            source: str = "manual_replay", plugin_version: str = "",
                            session_hash_override: str = "",
                            report: SampleBuildReport | None = None) -> list:
    """One annotation record in, up to three labelled samples out.

    Two facts are required before a row is emitted, and neither is guessed:

    * recipient samples need the bot id, or the router's choice cannot be read
      out of the trace and every row would be labelled "not the bot";
    * participation samples need a boolean should_reply on the trace, because a
      trace that recorded no decision is undecided, not silent.
    """
    sink = report if report is not None else SampleBuildReport()
    if not isinstance(record, Mapping):
        return []
    msg_id = str(record.get("msg_id") or "")
    if not msg_id:
        return []
    trace = record.get("decision_trace")
    trace = trace if isinstance(trace, Mapping) else {}
    routing = record.get("routing")
    routing = routing if isinstance(routing, Mapping) else {}
    features = features_from_trace(trace, routing, text=record.get("text") or "")
    stamp = record.get("annotated_at")
    stamp = float(stamp) if isinstance(stamp, (int, float)) and not isinstance(stamp, bool) else 0.0
    session = str(session_id or record.get("session_key") or "unknown")
    digest = (session_hash_override or str(record.get("session_hash") or "")
              or (session_hash(session) if session != "unknown" else ""))
    routing_version = trace.get("routing_schema_version")
    provenance = {
        "session_hash": digest,
        "policy_version": str(trace.get("weights_version") or ""),
        "plugin_version": str(plugin_version or ""),
        "routing_schema_version": (routing_version
                                   if isinstance(routing_version, int)
                                   and not isinstance(routing_version, bool) else 0),
        "feature_schema_version": FEATURE_SCHEMA_VERSION,
    }
    samples: list = []

    predicted_topic = str(record.get("predicted_topic") or "UNKNOWN")
    expected_topic = str(record.get("expected_topic") or "UNKNOWN")
    samples.append(LearningSample(
        session, msg_id, stamp, "topic", predicted_topic, expected_topic,
        _confidence(trace, "topic"), source, features, str(record.get("error_type") or ""),
        **provenance))

    expected_recipient = _recipient_label(record)
    predicted_recipient = _predicted_recipient(trace, bot_id)
    if expected_recipient is None:
        if "bot_targeted" in record:
            sink.note(SKIP_NO_RECIPIENT_LABEL)
    elif predicted_recipient is None:
        sink.note(SKIP_NO_BOT_ID)
    else:
        samples.append(LearningSample(
            session, msg_id, stamp, "recipient", predicted_recipient,
            expected_recipient, _confidence(trace, "recipient"), source, features,
            str(record.get("recipient_error_type") or ""), **provenance))

    expected_reply = record.get("expected_reply")
    should_reply = (trace.get("participation") or {}).get("should_reply")
    if not isinstance(expected_reply, bool):
        if isinstance(should_reply, bool):
            sink.note(SKIP_NO_REPLY_LABEL)
    elif not isinstance(should_reply, bool):
        # The trace allows null here, and null means undecided. Recording it as
        # "silent" would teach a learner that not deciding is deciding not to.
        sink.note(SKIP_UNDECIDED_REPLY)
    else:
        samples.append(LearningSample(
            session, msg_id, stamp, "participation",
            REPLY if should_reply else SILENT, REPLY if expected_reply else SILENT,
            _confidence(trace, "participation"), source, features,
            "correct" if should_reply is expected_reply else "wrong_reply_decision",
            **provenance))

    return samples


def samples_from_annotations(records, *, session_id: str = "", bot_id: str = "",
                             source: str = "manual_replay",
                             plugin_version: str = "") -> list:
    """Compatibility wrapper returning only the samples.

    Callers that need to know what was withheld should use build_samples.
    """
    return build_samples(records, session_id=session_id, bot_id=bot_id, source=source,
                         plugin_version=plugin_version).samples
